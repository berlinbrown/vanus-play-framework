...
final class VanusHomeMessagesHandler(
  ...
  private def page(content: String): String =
    s"""<!doctype html>
       ...
       |<title>Vanus Messages</title><style>
       ...
       |@media(max-width:600px){.hero{align-items:start;flex-direction:column;padding-top:42px}.message{max-width:94%}.message header{flex-direction:column;gap:2px}}
       |</style></head><body>$content</body></html>""".stripMargin
